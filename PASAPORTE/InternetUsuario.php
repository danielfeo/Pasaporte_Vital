...
<div id="main">
  ...
  <div id="site_contentidrd">
                       ...
                       <div id="templatemo_header_3">
					     ...
					     <p>&nbsp;</p>
						 <form id="form1" method="post" action="InternetUsuario.php">
						  <center>
						     <p>Cedula.
								<input type="text" name="CT_ID" id="CT_ID" />
				               <input type="submit" name="BT_Ver" id="BT_Ver" value="VER USUARIO" />
						     </p>
					         <p>&nbsp;</p>
                          </center>
					     </form>
  ...
  <?php
			...
			$Conn = conectar();
					  ...
					  if(!empty($_POST['CT_ID'])){
						$id=$_POST['CT_ID'];
					    $sql="SELECT B.`idPasaporte`, B.`documento`, A.`nombres` , A.`apellidos` , B.`fechaExpedicion` FROM pasaporte_registro A, pasaporte_pasaporte B   WHERE id_Operario=0 AND B.`documento`= A.`documento` AND B.`documento`='$id' AND B.`impreso`= 'NO';";   
					  }else{
						
						$sql="SELECT B.`idPasaporte`, B.`documento`, A.`nombres` , A.`apellidos` , B.`fechaExpedicion` FROM pasaporte_registro A, pasaporte_pasaporte B   WHERE id_Operario=0 AND B.`documento`= A.`documento` AND B.`impreso`= 'NO'; ";   
						...
						echo "<td><a href='Carnet2.php?id=".$row['documento']."'>Imprimir</a></td>";
						echo "</tr>";
						$var++;
					     } 
						 }else{ 
